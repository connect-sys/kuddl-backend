...
export function mapRowsToCareRaw(serviceRow = {}, providerRow = {}) {
  ...
  return {
    ...
    age_max: cp.age_max,
    mode: cp.mode,
    // Partner-entered details the customer should see (were collected but hidden).
    concerns: cp.concerns,
    qualification: cp.qualification,
    format: cp.format,
    typical_course: cp.typical_course,
    home_visits: cp.home_visits,
    home_visit_areas: cp.home_visit_areas,
    clinic_address: cp.clinic_address,
    availability_days: cp.availability_days,
...
export async function getCareServiceList(request, env) {
  try {
    ...
    const svcRes = await env.KUDDL_DB
      .prepare("SELECT * FROM services WHERE status = 'active' AND LOWER(category_id) LIKE '%care%' ORDER BY created_at DESC LIMIT 60")
      .all().catch(() => ({ results: [] }));
